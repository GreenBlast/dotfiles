...
Resolution strategy, per pane (ground truth, no guessing):
  1. the pane's live `claude` process argv has `--resume <x>`  -> use <x>
  2. else match the pane title to a session's ai-title / custom-title
     stored in ~/.claude/projects/<encoded-cwd>/*.jsonl       -> use that UUID
                ...
                for raw in fh:
                    if b'"ai-title"' in raw or b'"custom-title"' in raw:
                        ...
                        if o.get("type") == "ai-title" and o.get("aiTitle"):
                            ...
                            continue
                        titles = [x for x in titles if x != t] + [t]
        except Exception:
            continue
        for t in set(titles):
            prev = res.get(t)
...
tmaps = {}
for p in panes:
    if p["claude_cmd"] and not p["resume_name"] and p["cwd"] not in tmaps:
...
for p in panes:
    ...
    if p["resume_name"]:
        rows.append((p["addr"], p["cwd"], title, p["resume_name"], "named-arg"))
    else:
        hit = tmaps.get(p["cwd"], {}).get(title)
        if hit:
            rows.append((p["addr"], p["cwd"], title, hit[0], "title-match"))
        else:
            rows.append((p["addr"], p["cwd"], title, "", "UNRESOLVED"))
